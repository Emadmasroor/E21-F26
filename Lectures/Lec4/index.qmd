...
- Part of the so-called _[standard library](https://docs.python.org/3/library/index.html)_. Standard library packages must be imported.
  These include packages such as `random`, `math`, `calendar` and `time`.
- Available for download from the [Python package index](https://pypi.org/).  These must be downloaded at least once on a computer, and can subsequently be `import`ed.
  These include packages such as `numpy`, for numerical computing, or `matplotlib`, for plotting.
